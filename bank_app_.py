...
class BankingAppGUI(ctk.CTk):
    def __init__(self):
        ...
        # self.curr_date = datetime.datetime(2025,1,23).strftime("%Y-%m-%d")
    ...
    def submit_info(self):
        ...
        try:
            ...
            if not (len(self.phoneno)==10 and self.phoneno.isdigit()):
                ...
                messagebox.showerror("Error", "Phone No. must be of 10 digits!You have entered more than 10 digits")
                return
            elif len(self.phoneno)<10:
                messagebox.showerror("Error", "Phone No. must be of 10 digits!You have entered less than 10 digits")
                return
            else:
            ...
            if not (len(self.aadhaarno)==12 and self.aadhaarno.isdigit()):
                ...
                return
            elif len(self.aadhaarno)>12:
                messagebox.showerror("Error", "Aadhaar No. must be of 12 digits!You have entered more than 12 digits")
                return
            elif len(self.aadhaarno)<12:
                messagebox.showerror("Error", "Aadhaar No. must be of 12 digits!You have entered less than 12 digits")
                return
            else:
                self.valid_aadhaar=self.aadhaarno

            #  passowrd validation
            if not len(self.password)==4 and self.phoneno.isdigit():
                messagebox.showerror("Error", "Password must be of 4 digits only!")
                return
...
# ------------------------------------------------------------------- ADD COUNT TO VERIFY OTP ---------------------------------------
    ...
    def verify_otp(self):
        ...
        elif entered_otp != str(self.generated_otp):
            messagebox.showerror("Error", f"Invalid OTP!{self.count}")
            self.count+=1
        else:
            self.count+=1
            messagebox.showerror("Error", f"Invalid OTP! Try Again{self.count}")

        if self.count>3:
            self.count=0
            messagebox.showerror("Error", "You tried more than 3 times Sorry Verification was not possible ")
            self.clear_frame()
            self.show_main_menu()
    ...
    def show_after_login_options(self):
        ...
        balance_btn = ctk.CTkButton(self, text="Check Balance", command=self.show_check_balance)
    ...
    def show_check_balance(self):
        ...
        label = ctk.CTkLabel(self, text="Check Balance", font=("Arial", 20))
        label.pack(pady=20)

        self.password_entry = ctk.CTkEntry(self, placeholder_text="Password") 
        self.password_entry.pack(pady=5)

        chkps_btn = ctk.CTkButton(self, text="Check Balance", command=self.check_balance)
        chkps_btn.pack(pady=10)
    ...
    def check_balance(self):
        ...
        try:
            ...
            # fetches the password of the user
            ps_query = f'SELECT password FROM customer_info WHERE username ="{self.current_user}"'
            ...
            if ps==password:
                ...
                self.count+=1
                messagebox.showerror("Error", f"Wrong password {self.count}")
    ...
    def all_customers(self):
        t_balance="update customer_info set target_balance='>50K' where balance>50000"
        tg_balance="update customer_info set target_balance='<=50K' where balance<=50000"
        cur.execute(t_balance)
        cur.execute(tg_balance)
        db.commit()
        
        all='select * from customer_info'
        cur.execute(all)
        all_info=cur.fetchall()
        tablecolumns=["c_id","fname","mname","lname","gender","phoneno","email","aadhaarno","dob","age","occupation","monthly_income","marital_status","education_level","date","username","password","balance","target_balance"]
